...
import { Link2, ShieldCheck, AlertTriangle, CheckCircle, Loader2, ExternalLink } from "lucide-react";
...
export default function BrokerPage() {
  ...
  useEffect(() => { 
    ...
    // Check if we just returned from Stoxkart with an auth_token
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API}/api/broker/credentials`, {
        method: "POST",
      ...
      setMsg({ type: "success", text: "Credentials saved securely ✓. Now click Login to connect." });
    } catch (err: unknown) {
  ...
  const handleLoginClick = async () => {
    ...
    try {
      const res = await fetch(`${API}/api/broker/login-url`, {
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message);
  ...
  const handleConnectAfterRedirect = async (authToken: string) => {
    setConnecting(true); setMsg(null);
    try {
      const res = await fetch(`${API}/api/broker/connect`, {
        method: "POST",
        headers: { 
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message);
      setMsg({ type: "success", text: "Broker connected successfully! ✓" });
  ...
  const handleDisconnect = async () => {
    ...
    try {
      const res = await fetch(`${API}/api/broker/disconnect`, {
        method: "POST",
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message);
      setMsg({ type: "success", text: "Broker disconnected" });
      await fetchStatus();
    } catch (err: unknown) {
      setMsg({ type: "error", text: err instanceof Error ? err.message : "Failed" });
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            <button
              type="submit"
              disabled={saving}
              className="flex items-center gap-2 px-4 py-2.5 bg-gray-700 hover:bg-gray-600 text-white rounded-lg text-sm font-medium transition-all disabled:opacity-60"
            >
            ...
            <button
              ...
              disabled={connecting}
              className="flex items-center gap-2 px-4 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-white rounded-lg text-sm font-semibold transition-all disabled:opacity-60 shadow-lg shadow-emerald-500/20"
            >
      ...
      <div className="bg-gray-900/40 border border-gray-800 rounded-xl p-5 space-y-4">
        ...
        <div className="space-y-3 text-xs text-gray-500">
          <div className="flex gap-3">
            <span className="flex-shrink-0 w-5 h-5 rounded-full bg-gray-800 flex items-center justify-center text-gray-400">1</span>
            <p>Login to your <a href="https://superrtrade.stoxkart.com" target="_blank" className="text-emerald-400 hover:underline">Stoxkart SuperrTrade</a> portal.</p>
          </div>
          <div className="flex gap-3">
            <span className="flex-shrink-0 w-5 h-5 rounded-full bg-gray-800 flex items-center justify-center text-gray-400">2</span>
            <p>Go to <strong>Settings</strong> or <strong>My Profile</strong> section from the top-right menu.</p>
          </div>
          <div className="flex gap-3">
            <span className="flex-shrink-0 w-5 h-5 rounded-full bg-gray-800 flex items-center justify-center text-gray-400">3</span>
            <p>Look for the <strong>API / Developer Options</strong> tab.</p>
          </div>
          <div className="flex gap-3">
            <span className="flex-shrink-0 w-5 h-5 rounded-full bg-gray-800 flex items-center justify-center text-gray-400">4</span>
            <p>Click on <strong>Generate API Key</strong>. You will receive an <strong>API Key</strong> and an <strong>API Secret</strong>.</p>
          </div>
          <div className="flex gap-3">
            <span className="flex-shrink-0 w-5 h-5 rounded-full bg-gray-800 flex items-center justify-center text-gray-400">5</span>
            <p>Enter those keys in the form above and click <strong>Save Credentials</strong> before connecting.</p>
          </div>
        </div>
      </div>
